...
const useBroadcast = (
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent<any>) => {
      const selfContext = sessionStorage.getItem(`${channelName}_bc_self_context`) || ""
      const bcContext = localStorage.getItem(`${channelName}_bc_context`) || ""
    ...
    let bcContext = localStorage.getItem(`${channelName}_bc_context`) || ""
    let selfContext = sessionStorage.getItem(`${channelName}_bc_self_context`) || ""
    if (!selfContext) {
      if (!bcContext) {
        localStorage.setItem(`${channelName}_bc_context`, "1")
        sessionStorage.setItem(`${channelName}_bc_self_context`, "1")
      }
    }
    if (!selfContext && bcContext === "1") {
      sessionStorage.setItem(`${channelName}_bc_self_context`, `${parseInt(bcContext) + 1}`)
    }
    if (!selfContext && !bcContext) {
      sessionStorage.setItem(`${channelName}_bc_self_context`, `${1}`)
    }
    selfContext = sessionStorage.getItem(`${channelName}_bc_self_context`) || ""
    bcContext = localStorage.getItem(`${channelName}_bc_context`) || ""
    bc.current = new BroadcastChannel(channelName)
    bc.current.onmessage = (event) => onMessage(event)
    return () => {
      if (bcContext === "1" && selfContext === "1") {
        localStorage.removeItem(`${channelName}_bc_context`)
      }
      sessionStorage.removeItem(`${channelName}_bc_self_context`)
      bc.current?.close()
